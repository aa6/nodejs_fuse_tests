[`fuse-loopback-readwrite`](/fuse-loopback-readwrite.coffee) is a moderate-featured loopback linux-only filesystem implementation for a Node.js [`fuse-bindings`](https://github.com/mafintosh/fuse-bindings) package. <ins>It consists of 20 functions. </ins> <ins>The functions are not binded upon mounting into some class instance and `this` by default points to a higher context, usually to `global`<sup>[*](/spec/fuse-loopback-readwrite.spec.coffee#L11)[*](/spec/fuse-loopback-readwrite.spec.coffee#L175)[*](/spec/fuse-loopback-readwrite.spec.coffee#L269)[*](/spec/fuse-loopback-readwrite.spec.coffee#L299)[*](/spec/fuse-loopback-readwrite.spec.coffee#L327)[*](/spec/fuse-loopback-readwrite.spec.coffee#L350)</sup>.</ins>

This text is backed and generated by [the test in `spec` folder](/spec/fuse-loopback-readwrite.spec.coffee)

### init(cb)
<ins>Called on filesystem initialization, prior to all other functions.</ins> Called only one time per each mount. Always accepts only one input argument.
