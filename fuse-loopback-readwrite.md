...
### init(cb)
Called on filesystem initialization, prior to all other functions.<sup>[L360](/spec/fuse-loopback-readwrite.spec.coffee#L360)</sup> Called only one time per each mount. Always accepts only one input argument.
