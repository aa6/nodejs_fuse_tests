...
####################################################################################################
  it "matches the preformatted description", ->
    ...
    generate_formatted_description = ->
      """
      `fuse-loopback-readwrite` is a moderate-featured loopback filesystem linux-only \
      implementation for a Node.js `fuse-bindings` package. It consists of #{Object.keys(\
      loopbackfs_instance).length} functions. #{if expectations["fuse implementation functions are \
      not integrated by the `this` context"] then "The functions are not binded upon mounting into \
      some class instance and `this` by default points to a higher context, usually to `global`." \
      else ERRSTR}
