...
####################################################################################################
  it "matches the preformatted description", ->
    ...
    generate_formatted_description = ->
      ...
      functions. #{if expectations["fuse implementation functions are not integrated by the `this` \
      context"] then "The functions are not binded upon mounting into some class instance and \
      `this` by default points to a higher context, usually to `global`." else ERRSTR}
      ...
      **Return:**  
      ...
      doesn't exists"] then "Readdir is not called if the `path` entry doesn't exists." else \
      ERRSTR} #{if expectations["readdir always accepts only 2 arguments"] then "Always accepts \
      only two input arguments." else ERRSTR}
