...
####################################################################################################
  it "matches the preformatted description", ->
    ...
    generate_formatted_description = ->
      ...
      ### init(cb)
      #{if expectations["init is called prior to all other functions"] then "Called on filesystem \
      initialization, prior to all other functions." else ERRSTR} #{if expectations["init is \
