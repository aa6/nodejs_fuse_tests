...
####################################################################################################
  it "matches the preformatted description", ->
    ...
    refgen = (text, expectkeys, condition) ->
      result = ""
    ...
    generate_formatted_description = ->
      ...
      ### init(cb)
      #{if expectations["init is called prior to all other functions"] then "<ins>Called on filesystem \
      initialization, prior to all other functions.</ins>" else ERRSTR} #{if expectations["init is \
